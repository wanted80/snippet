...
Use an exact release tag for reproducible output. The examples pin v2.1.2. <!-- x-release-please-version --> `--user` prevents root-owned output, while `--volume` exposes the current repository at the image's `/workspace` path:
...
docker run --rm \
  ...
  ghcr.io/wanted80/snippet:v2.1.2 init # x-release-please-version
```
...
docker run --rm \
  --user "$(id -u):$(id -g)" \
  --volume "$PWD:/workspace" \
  ghcr.io/wanted80/snippet:v2.1.2 new page contact # x-release-please-version

docker run --rm \
  --user "$(id -u):$(id -g)" \
  --volume "$PWD:/workspace" \
  ghcr.io/wanted80/snippet:v2.1.2 new article first-post # x-release-please-version
```
...
Moving release aliases and `latest` are convenient for evaluation but unsuitable for reproducible publication. Pin a full release such as `v2.1.2` or an immutable image digest. <!-- x-release-please-version -->
...
docker run --rm \
  ...
  ghcr.io/wanted80/snippet:v2.1.2 build # x-release-please-version
```
      ...
      - name: Validate
        run: |
          docker run --rm --network none --read-only --cap-drop ALL \
            ...
            --tmpfs /tmp:rw,noexec,nosuid,nodev,size=16m \
            ghcr.io/wanted80/snippet:v2.1.2 validate # x-release-please-version
      - name: Build
        run: |
          docker run --rm --network none --read-only --cap-drop ALL \
            --security-opt no-new-privileges --user "$(id -u):$(id -g)" \
