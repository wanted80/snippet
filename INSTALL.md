...
Use an exact release tag for reproducible output. The examples pin v3.0.0. <!-- x-release-please-version --> `--user` prevents root-owned output, while `--volume` exposes the current repository at the image's `/workspace` path:
...
docker run --rm \
  ...
  ghcr.io/wanted80/snippet:v3.0.0 init # x-release-please-version
```
...
docker run --rm \
  --user "$(id -u):$(id -g)" \
  --volume "$PWD:/workspace" \
  ghcr.io/wanted80/snippet:v3.0.0 new page contact # x-release-please-version

docker run --rm \
  --user "$(id -u):$(id -g)" \
  --volume "$PWD:/workspace" \
  ghcr.io/wanted80/snippet:v3.0.0 new article first-post # x-release-please-version
```
...
Moving release aliases and `latest` are convenient for evaluation but unsuitable for reproducible publication. Pin a full release such as `v3.0.0` or an immutable image digest. <!-- x-release-please-version -->
...
docker run --rm \
  ...
  ghcr.io/wanted80/snippet:v3.0.0 build # x-release-please-version
```
...
docker run --rm --init \
  ...
  --mount type=bind,src="$PWD",dst=/workspace \
  --tmpfs /tmp:rw,noexec,nosuid,nodev,size=16m \
  ghcr.io/wanted80/snippet:v3.0.0 preview --host=0.0.0.0 --port=8080 # x-release-please-version
```
...
services:
  snippet:
    image: ghcr.io/wanted80/snippet:v3.0.0 # x-release-please-version
...
env:
  SNIPPET_IMAGE: ghcr.io/wanted80/snippet:v3.0.0 # x-release-please-version
