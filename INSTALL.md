...
Use an exact release tag for reproducible output. The examples pin v3.1.0. <!-- x-release-please-version --> `--user` prevents root-owned output, while `--volume` exposes the current repository at the image's `/workspace` path:
...
docker run --rm \
  --user "$(id -u):$(id -g)" \
  --volume "$PWD:/workspace" \
  ghcr.io/wanted80/snippet:v3.1.0 new page contact # x-release-please-version
...
Moving release aliases and `latest` are convenient for evaluation but unsuitable for reproducible publication. Pin a full release such as `v3.1.0` or an immutable image digest. <!-- x-release-please-version -->
...
docker run --rm \
  ...
  ghcr.io/wanted80/snippet:v3.1.0 build # x-release-please-version
```
...
services:
  snippet:
    image: ghcr.io/wanted80/snippet:v3.1.0 # x-release-please-version
...
env:
  SNIPPET_IMAGE: ghcr.io/wanted80/snippet:v3.1.0 # x-release-please-version
