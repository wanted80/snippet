...
Use an exact release tag for reproducible output. The examples pin v4.0.0. <!-- x-release-please-version --> `--user` prevents root-owned output, while `--volume` exposes the current repository at the image's `/workspace` path:
...
docker run --rm \
  ...
  ghcr.io/wanted80/snippet:v4.0.0 init # x-release-please-version
```
...
docker run --rm \
  --user "$(id -u):$(id -g)" \
  --volume "$PWD:/workspace" \
  ghcr.io/wanted80/snippet:v4.0.0 new page contact # x-release-please-version

docker run --rm \
  --user "$(id -u):$(id -g)" \
  --volume "$PWD:/workspace" \
  ghcr.io/wanted80/snippet:v4.0.0 new article first-post # x-release-please-version
```
...
Moving release aliases and `latest` are convenient for evaluation but unsuitable for reproducible publication. Pin a full release such as `v4.0.0` or an immutable image digest. <!-- x-release-please-version -->
...
docker run --rm \
  ...
  ghcr.io/wanted80/snippet:v4.0.0 build # x-release-please-version
```
...
docker run --rm --init \
  ...
  --mount type=bind,src="$PWD",dst=/workspace \
  --tmpfs /tmp:rw,noexec,nosuid,nodev,size=16m \
  ghcr.io/wanted80/snippet:v4.0.0 preview --host=0.0.0.0 --port=8080 # x-release-please-version
```
...
services:
  snippet:
    image: ghcr.io/wanted80/snippet:v4.0.0 # x-release-please-version
...
| --- | --- | --- |
| `resources/theme.css` | `/assets/theme.<xxh3>.css` | Required built-in theme; always conservatively minified, then fingerprinted from the published bytes. |
| `resources/theme.js` | `/assets/theme.<xxh3>.js` | Required progressive enhancement; copied unchanged and fingerprinted. |
| `site/site.css` | `/assets/site.<xxh3>.css` | Optional site CSS; loaded after the built-in theme, always conservatively minified, then fingerprinted from the published bytes. |
| `site/site.js` | `/assets/site.<xxh3>.js` | Optional local script; copied unchanged, fingerprinted, and loaded with `defer` after the built-in script. |

CSS entry files are always compacted in direct builds, Docker, and preview. HTML is published as rendered and JavaScript is copied unchanged. There is no minification setting: remove the obsolete `build` entry from existing `site/config.php` files. See the [minification scope](README.md#site-customization) for details.
...
env:
  SNIPPET_IMAGE: ghcr.io/wanted80/snippet:v4.0.0 # x-release-please-version
